...
TreeBillboardsApp::TreeBillboardsApp(HINSTANCE hInst)
	: D3DApp(hInst)
{
...
void TreeBillboardsApp::Draw(const GameTimer& timer)
{
	...
	else
	{
		...
		// Draw tree billboard render items.
		m_CommandList->SetPipelineState(m_PSOs["TreeBillboard"].Get());
		DrawRenderItems(m_CommandList.Get(), m_RenderableLayer[(int)RenderLayer::AlphaTestedTreeSprites]);
...
void TreeBillboardsApp::OnKeyboardInput(const GameTimer& timer)
{
...
void TreeBillboardsApp::AnimateMaterials(const GameTimer& timer)
{
...
void TreeBillboardsApp::UpdateCamera(const GameTimer& timer)
{
...
void TreeBillboardsApp::UpdateObjectCBs(const GameTimer& timer)
{
...
void TreeBillboardsApp::UpdateMaterialCBs(const GameTimer& timer)
{
...
void TreeBillboardsApp::UpdateMainPassCB(const GameTimer& timer)
{
...
void TreeBillboardsApp::UpdateWaves(const GameTimer& timer)
{
...
void TreeBillboardsApp::LoadTextures()
{
	...
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(m_d3dDevice.Get(),
		m_CommandList.Get(), treeArrayTex->Filename.c_str(),
...
void TreeBillboardsApp::BuildRenderItems()
{
	...
	auto treeSpritesItem = std::make_unique<Renderable>();
	treeSpritesItem->ObjCBIndex = 3;
	treeSpritesItem->Mat = m_Materials["treeSprites"].get();
	treeSpritesItem->Geo = m_Geometries["treeSpritesGeo"].get();
	treeSpritesItem->PrimitiveType = D3D10_PRIMITIVE_TOPOLOGY_POINTLIST;
	treeSpritesItem->IndexCount = treeSpritesItem->Geo->DrawArgs["points"].IndexCount;
	treeSpritesItem->StartIndexLocation = treeSpritesItem->Geo->DrawArgs["points"].StartIndexLocation;
	treeSpritesItem->BaseVertexLocation = treeSpritesItem->Geo->DrawArgs["points"].BaseVertexLocation;
	treeSpritesItem->World = MathHelper::Identity4x4();
	m_RenderableLayer[(int)RenderLayer::AlphaTestedTreeSprites].push_back(treeSpritesItem.get());
	...
	m_AllRenderables.push_back(std::move(treeSpritesItem));
}

void TreeBillboardsApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<Renderable*>& renderables)
{
...
void TreeBillboardsApp::BuildShadersAndInputLayout()                       
{
	...
	m_Shaders["TreeBillboardVS"] = D3DUtil::CompileShader(L"Shaders/" + projectName + L"/TreeBillboard_VS.hlsl", nullptr, "VS", "vs_5_0");
	m_Shaders["TreeBillboardGS"] = D3DUtil::CompileShader(L"Shaders/" + projectName + L"/TreeBillboard_GS.hlsl", nullptr, "GS", "gs_5_0");
	m_Shaders["TreeBillboardPS"] = D3DUtil::CompileShader(L"Shaders/" + projectName + L"/TreeBillboard_PS.hlsl", alphaTestedDefines, "PS", "ps_5_0");
...
void TreeBillboardsApp::BuildPSOs()
{
	...
	treeBillboardPsoDesc.InputLayout = { m_TreeSpriteInputLayout.data(), (UINT)m_TreeSpriteInputLayout.size() };
	...
	ThrowIfFailed(m_d3dDevice->CreateGraphicsPipelineState(&treeBillboardPsoDesc, IID_PPV_ARGS(&m_PSOs["TreeBillboard"])));
