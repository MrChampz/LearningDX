...
MeshExplosionApp::MeshExplosionApp(HINSTANCE hInst)
	: D3DApp(hInst)
{

}

MeshExplosionApp::~MeshExplosionApp()
{

}

bool MeshExplosionApp::Initialize()
{
...
void MeshExplosionApp::OnResize()
{
	D3DApp::OnResize();
...
void MeshExplosionApp::Update(const GameTimer& timer)
{
...
void MeshExplosionApp::Draw(const GameTimer& timer)
{
	...
	if (m_IsWireframe)
	{
		// Draw explosion render items as wireframe.
		DrawRenderItems(m_CommandList.Get(), m_RenderableLayer[(int)RenderLayer::Explosion]);
	}
	else
	{
		// Draw explosion render items.
		m_CommandList->SetPipelineState(m_PSOs["Explosion"].Get());
		DrawRenderItems(m_CommandList.Get(), m_RenderableLayer[(int)RenderLayer::Explosion]);
	}
...
void MeshExplosionApp::OnMouseDown(WPARAM btnState, int x, int y)
{
...
void MeshExplosionApp::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
...
void MeshExplosionApp::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void MeshExplosionApp::OnKeyboardInput(const GameTimer& timer)
{
...
void MeshExplosionApp::UpdateCamera(const GameTimer& timer)
{
...
void MeshExplosionApp::UpdateObjectCBs(const GameTimer& timer)
{
...
void MeshExplosionApp::UpdateMaterialCBs(const GameTimer& timer)
{
...
void MeshExplosionApp::UpdateMainPassCB(const GameTimer& timer)
{
...
float MeshExplosionApp::GetHillsHeight(float x, float z) const
{
	return 0.3f * (z * sinf(0.1f * x) + x * cosf(0.1f * z));
}

XMFLOAT3 MeshExplosionApp::GetHillsNormal(float x, float z) const
{
	// n = (-df/dx, 1, -df/dz)
	XMFLOAT3 n
	(
...
void MeshExplosionApp::LoadTextures()
{
...
void MeshExplosionApp::BuildIcosahedronGeometry()
{
...
void MeshExplosionApp::BuildMaterials()
{
...
void MeshExplosionApp::BuildRenderItems()
{
	...
	m_RenderableLayer[(int)RenderLayer::Explosion].push_back(icoItem.get());

	m_AllRenderables.push_back(std::move(icoItem));
}

void MeshExplosionApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<Renderable*>& renderables)
{
...
void MeshExplosionApp::BuildFrameResources()
{
...
void MeshExplosionApp::BuildDescriptorHeaps()
{
	// Create the SRV heap.
...
void MeshExplosionApp::BuildRootSignature()
{
...
void MeshExplosionApp::BuildShadersAndInputLayout()
{
	...
	m_Shaders["ExplosionVS"] = D3DUtil::CompileShader(L"../Shaders/Explosion_VS.hlsl", nullptr, "VS", "vs_5_0");
	m_Shaders["ExplosionGS"] = D3DUtil::CompileShader(L"../Shaders/Explosion_GS.hlsl", nullptr, "GS", "gs_5_0");
	m_Shaders["ExplosionPS"] = D3DUtil::CompileShader(L"../Shaders/Explosion_PS.hlsl", alphaTestedDefines, "PS", "ps_5_0");
...
void MeshExplosionApp::BuildPSOs()
{
	...
	//
	// PSO for mesh explosion into geometry shader.
	//
	D3D12_GRAPHICS_PIPELINE_STATE_DESC explosionPsoDesc = opaquePsoDesc;
	explosionPsoDesc.VS =
	{
		reinterpret_cast<BYTE*>(m_Shaders["ExplosionVS"]->GetBufferPointer()),
		m_Shaders["ExplosionVS"]->GetBufferSize()
	};
	explosionPsoDesc.GS =
	{
		reinterpret_cast<BYTE*>(m_Shaders["ExplosionGS"]->GetBufferPointer()),
		m_Shaders["ExplosionGS"]->GetBufferSize()
	};
	explosionPsoDesc.PS =
	{
		reinterpret_cast<BYTE*>(m_Shaders["ExplosionPS"]->GetBufferPointer()),
		m_Shaders["ExplosionPS"]->GetBufferSize()
	};
	explosionPsoDesc.RasterizerState.CullMode = D3D12_CULL_MODE_NONE;

	ThrowIfFailed(m_d3dDevice->CreateGraphicsPipelineState(&explosionPsoDesc, IID_PPV_ARGS(&m_PSOs["Explosion"])));
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 6> MeshExplosionApp::GetStaticSamplers()
{
