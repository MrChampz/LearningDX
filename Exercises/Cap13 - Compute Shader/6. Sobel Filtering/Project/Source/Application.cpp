...
bool Application::Initialize()
{
	...
	m_SobelFilter = std::make_unique<SobelFilter>(
		m_d3dDevice.Get(), m_ClientWidth, m_ClientHeight, m_BackBufferFormat);
...
void Application::Draw(const GameTimer& timer)
{
	...
	// Bind the PSO for opaque rendering.
	...
	m_CommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(
		m_OffscreenRT->GetResource(), D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_STATE_RENDER_TARGET));

	// Clear the back buffer and depth buffer.
	m_CommandList->ClearRenderTargetView(m_OffscreenRT->GetRtv(), (float*)&m_MainPassCB.FogColor, 0, nullptr);
	...
	m_CommandList->OMSetRenderTargets(1, &m_OffscreenRT->GetRtv(), true, &GetDepthStencilView());
	...
	m_CommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(
		m_OffscreenRT->GetResource(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_GENERIC_READ));

	m_SobelFilter->Execute(
		m_CommandList.Get(),
		m_PostProcessRootSignature.Get(),
		m_PSOs["Sobel"].Get(),
		m_OffscreenRT->GetSrv());
	...
	m_CommandList->SetGraphicsRootDescriptorTable(1, m_SobelFilter->GetOutputSrv());
...
{
	const int textDescCount = 3;
	const int sobelDescCount = m_SobelFilter->GetDescriptorCount();
	const int wavesDescCount = m_Waves->GetDescriptorCount();
	...
	m_Waves->BuildDescriptors(
		CD3DX12_CPU_DESCRIPTOR_HANDLE(m_CbvSrvUavDescriptorHeap->GetCPUDescriptorHandleForHeapStart(), offset, m_CbvSrvUavDescriptorSize),
		CD3DX12_GPU_DESCRIPTOR_HANDLE(m_CbvSrvUavDescriptorHeap->GetGPUDescriptorHandleForHeapStart(), offset, m_CbvSrvUavDescriptorSize),
		m_CbvSrvUavDescriptorSize);

	offset += wavesDescCount;
...
void Application::BuildShadersAndInputLayout()                       
{
	...
	m_Shaders["CompositeVS"] = D3DUtil::CompileShader(L"../Shaders/Composite_VS.hlsl", nullptr, "VS", "vs_5_0");
	m_Shaders["CompositePS"] = D3DUtil::CompileShader(L"../Shaders/Composite_PS.hlsl", nullptr, "PS", "ps_5_0");
	m_Shaders["SobelCS"] = D3DUtil::CompileShader(L"../Shaders/Sobel_CS.hlsl", nullptr, "SobelCS", "cs_5_0");
...
void Application::BuildPSOs()
{
	...
	//
	// PSO for Sobel filtering.
	//

	D3D12_COMPUTE_PIPELINE_STATE_DESC sobelFilteringPsoDesc = {};
	sobelFilteringPsoDesc.pRootSignature = m_PostProcessRootSignature.Get();
	sobelFilteringPsoDesc.CS = 
	{
		reinterpret_cast<BYTE*>(m_Shaders["SobelCS"]->GetBufferPointer()),
		m_Shaders["SobelCS"]->GetBufferSize()
	};
	sobelFilteringPsoDesc.Flags = D3D12_PIPELINE_STATE_FLAG_NONE;

	ThrowIfFailed(m_d3dDevice->CreateComputePipelineState(&sobelFilteringPsoDesc, IID_PPV_ARGS(&m_PSOs["Sobel"])));
}
