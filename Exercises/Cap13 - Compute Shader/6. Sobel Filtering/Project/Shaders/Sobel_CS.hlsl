...
// Approximates Luminance ("brightness") from an RGB value.  These weights are derived
// from experiment based on eye sensitivity to different wavelenghts of light.
...
void SobelCS(int3 DTid : SV_DispatchThreadID)
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        for (int j = 0; j < 3; ++j)
        {
            int2 xy = DTid.xy + int2(-1.0f + j, -1.0f + i);
            c[i][j] = gInput[xy];
        }
    }
    ...
    // Gradient is (Gx, Gy). For each color channel, compute
    // magnitude to get maximum rate of change.
