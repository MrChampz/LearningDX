...
{
	// Generate some data.
	std::vector<XMFLOAT3> data(NumVectors);
	for (int i = 0; i < NumVectors; ++i)
	{
		float y = (float)MathHelper::Rand(1, 10);
		data[i] = XMFLOAT3(0.0f, y, 0.0f);
	}
	...
	ThrowIfFailed(m_d3dDevice->CreateCommittedResource(
		...
		&CD3DX12_RESOURCE_DESC::Buffer(data.size() * sizeof(float), D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS),
	...
	ThrowIfFailed(m_d3dDevice->CreateCommittedResource(
		...
		D3D12_HEAP_FLAG_NONE,
		&CD3DX12_RESOURCE_DESC::Buffer(data.size() * sizeof(float)),
