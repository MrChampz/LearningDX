...
void Application::DoComputeWork()
{
	...
	m_CommandList->SetComputeRootUnorderedAccessView(0, m_InputBuffer->GetGPUVirtualAddress());
	...
	ThrowIfFailed(m_ReadBackBuffer->Map(0, &CD3DX12_RANGE(0, 0), reinterpret_cast<void**>(&mappedData)));

	std::ofstream fout("Results.txt");

	for (int i = 0; i < NumVectors; ++i)
	{
		fout << mappedData[i].v.x << " " << mappedData[i].v.y << " " << mappedData[i].v.z << std::endl;
	}
...
void Application::BuildBuffers()
{
	...
	// Create the input buffer to be used as UAV.
	...
	subres.RowPitch = byteSize;
	subres.SlicePitch = byteSize;

	UpdateSubresources<1>(m_CommandList.Get(), m_InputBuffer.Get(), m_InputUploadBuffer.Get(), 0, 0, 1, &subres);

	m_CommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(
		m_InputBuffer.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_UNORDERED_ACCESS));

	// Create the output buffer to be used as UAV.
	ThrowIfFailed(m_d3dDevice->CreateCommittedResource(
		&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT),
		D3D12_HEAP_FLAG_NONE,
		&CD3DX12_RESOURCE_DESC::Buffer(byteSize, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS),
