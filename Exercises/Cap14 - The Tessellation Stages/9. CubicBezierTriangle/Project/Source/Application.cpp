...
void Application::Draw(const GameTimer& timer)
{
	...
	if (m_ShowControlPoints)
	{
		...
		DrawRenderItems(m_CommandList.Get(), m_RenderableLayer[(int)RenderLayer::ControlPoint]);
	}
...
void Application::OnKeyboardInput(const GameTimer& timer)
{
	if (GetAsyncKeyState('1') & 0x8000)
		if (m_ShowControlPoints != true) m_ShowControlPoints = true;
	if (GetAsyncKeyState('2') & 0x8000)
		if (m_ShowControlPoints != false) m_ShowControlPoints = false;
}
...
void Application::BuildTrianglePatchGeometry()
{
	std::array<XMFLOAT3, 10> vertices =
	{
		// Row 0
		XMFLOAT3(-15.0f, -15.0f,  15.0f),
...
void Application::BuildSphereGeometry()
{
	GeometryGenerator::MeshData sphere = GeometryGenerator::CreateSphere(0.25f, 20, 4);
...
void Application::BuildRenderItems()
{
	...
	quadPatchItem->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_10_CONTROL_POINT_PATCHLIST;
	...
	std::array<XMFLOAT3, 10> controlPoints =
	{
		// Row 0
		XMFLOAT3(-15.0f, -15.0f,  15.0f),
		XMFLOAT3(-5.0f,   0.0f,  15.0f),
	...
	{
		auto sphereItem = std::make_unique<Renderable>();
		sphereItem->ObjCBIndex = i + 1;
		sphereItem->Geo = m_Geometries["sphereGeo"].get();
		...
		XMStoreFloat4x4(&sphereItem->World, XMMatrixTranslation(controlPoints[i].x, controlPoints[i].y, controlPoints[i].z));
		m_RenderableLayer[(int)RenderLayer::ControlPoint].push_back(sphereItem.get());

		m_AllRenderables.push_back(std::move(sphereItem));
	}
}
...
{
	m_Shaders["OpaqueVS"] = D3DUtil::LoadBinary(L"Shaders/Opaque_VS.cso");
...
void Application::BuildPSOs()
{
	...
	//
	// PSO for control points rendering.
	//
	...
	opaquePsoDesc.VS =
	{
		reinterpret_cast<BYTE*>(m_Shaders["OpaqueVS"]->GetBufferPointer()),
		m_Shaders["OpaqueVS"]->GetBufferSize()
	};
	opaquePsoDesc.PS =
	{
