...
void Application::BuildTrianglePatchGeometry()
{
	...
	geo->DrawArgs["triangle"] = quadSubmesh;
...
{
	auto trianglePatchItem = std::make_unique<Renderable>();
	trianglePatchItem->ObjCBIndex = 0;
	trianglePatchItem->Geo = m_Geometries["triangleGeo"].get();
	trianglePatchItem->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_3_CONTROL_POINT_PATCHLIST;
	trianglePatchItem->IndexCount = trianglePatchItem->Geo->DrawArgs["triangle"].IndexCount;
	trianglePatchItem->StartIndexLocation = trianglePatchItem->Geo->DrawArgs["triangle"].StartIndexLocation;
	trianglePatchItem->BaseVertexLocation = trianglePatchItem->Geo->DrawArgs["triangle"].BaseVertexLocation;
	m_RenderableLayer[(int)RenderLayer::Opaque].push_back(trianglePatchItem.get());

	m_AllRenderables.push_back(std::move(trianglePatchItem));
}
