...
void Application::BuildIcosahedronPatchGeometry()
{
	GeometryGenerator::MeshData ico = GeometryGenerator::CreateGeosphere(1.0f, 0);
...
{
	auto icoPatchItem = std::make_unique<Renderable>();
	icoPatchItem->ObjCBIndex = 0;
	icoPatchItem->Geo = m_Geometries["icoGeo"].get();
	icoPatchItem->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_3_CONTROL_POINT_PATCHLIST;
	icoPatchItem->IndexCount = icoPatchItem->Geo->DrawArgs["ico"].IndexCount;
	icoPatchItem->StartIndexLocation = icoPatchItem->Geo->DrawArgs["ico"].StartIndexLocation;
	icoPatchItem->BaseVertexLocation = icoPatchItem->Geo->DrawArgs["ico"].BaseVertexLocation;
	m_RenderableLayer[(int)RenderLayer::Opaque].push_back(icoPatchItem.get());

	m_AllRenderables.push_back(std::move(icoPatchItem));
}
...
{
	m_Shaders["IcosahedronVS"] = D3DUtil::LoadBinary(L"Shaders/Icosahedron_VS.cso");
	m_Shaders["IcosahedronHS"] = D3DUtil::LoadBinary(L"Shaders/Icosahedron_HS.cso");
	m_Shaders["IcosahedronDS"] = D3DUtil::LoadBinary(L"Shaders/Icosahedron_DS.cso");
	m_Shaders["IcosahedronPS"] = D3DUtil::LoadBinary(L"Shaders/Icosahedron_PS.cso");
...
{
	D3D12_GRAPHICS_PIPELINE_STATE_DESC icoPsoDesc;
	ZeroMemory(&icoPsoDesc, sizeof(D3D12_GRAPHICS_PIPELINE_STATE_DESC));

	//
	// PSO for tessellating objects.
	//

	icoPsoDesc.InputLayout = { m_InputLayout.data(), (UINT)m_InputLayout.size() };
	icoPsoDesc.pRootSignature = m_RootSignature.Get();
	icoPsoDesc.VS =
	{
		reinterpret_cast<BYTE*>(m_Shaders["IcosahedronVS"]->GetBufferPointer()),
		m_Shaders["IcosahedronVS"]->GetBufferSize()
	};
	icoPsoDesc.HS =
	{
		reinterpret_cast<BYTE*>(m_Shaders["IcosahedronHS"]->GetBufferPointer()),
		m_Shaders["IcosahedronHS"]->GetBufferSize()
	};
	icoPsoDesc.DS =
	{
		reinterpret_cast<BYTE*>(m_Shaders["IcosahedronDS"]->GetBufferPointer()),
		m_Shaders["IcosahedronDS"]->GetBufferSize()
	};
	icoPsoDesc.PS =
	{
		reinterpret_cast<BYTE*>(m_Shaders["IcosahedronPS"]->GetBufferPointer()),
		m_Shaders["IcosahedronPS"]->GetBufferSize()
	};
	icoPsoDesc.RasterizerState = CD3DX12_RASTERIZER_DESC(D3D12_DEFAULT);
	icoPsoDesc.RasterizerState.FillMode = D3D12_FILL_MODE_WIREFRAME;
	icoPsoDesc.RasterizerState.CullMode = D3D12_CULL_MODE_NONE;
	icoPsoDesc.BlendState = CD3DX12_BLEND_DESC(D3D12_DEFAULT);
	icoPsoDesc.DepthStencilState = CD3DX12_DEPTH_STENCIL_DESC(D3D12_DEFAULT);
	icoPsoDesc.SampleMask = UINT_MAX;
	icoPsoDesc.PrimitiveTopologyType = D3D12_PRIMITIVE_TOPOLOGY_TYPE_PATCH;
	icoPsoDesc.NumRenderTargets = 1;
	icoPsoDesc.RTVFormats[0] = m_BackBufferFormat;
	icoPsoDesc.DSVFormat = m_DepthStencilFormat;
	icoPsoDesc.SampleDesc.Count = m_msaaState ? m_msaaLevel : 1;
	icoPsoDesc.SampleDesc.Quality = m_msaaState ? (m_msaaQuality - 1) : 0;
	
	ThrowIfFailed(m_d3dDevice->CreateGraphicsPipelineState(&icoPsoDesc, IID_PPV_ARGS(&m_PSOs["Tessellation"])));
}