...
BlurApp::BlurApp(HINSTANCE hInst)
	: D3DApp(hInst)
{
...
bool BlurApp::Initialize()
{
	...
	m_BlurFilter = std::make_unique<BlurFilter>(
		m_d3dDevice.Get(), m_ClientWidth, m_ClientHeight, DXGI_FORMAT_R8G8B8A8_UNORM);
...
void BlurApp::Update(const GameTimer& timer)
{
...
void BlurApp::Draw(const GameTimer& timer)
{
	...
	m_CommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(
		GetCurrentBackBuffer(), D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_COPY_DEST));

	m_CommandList->CopyResource(GetCurrentBackBuffer(), m_BlurFilter->GetOutput());

	// Transition to PRESENT state
	m_CommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(
		GetCurrentBackBuffer(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PRESENT));
...
void BlurApp::OnKeyboardInput(const GameTimer& timer)
{
...
void BlurApp::AnimateMaterials(const GameTimer& timer)
{
...
void BlurApp::UpdateCamera(const GameTimer& timer)
{
...
void BlurApp::UpdateObjectCBs(const GameTimer& timer)
{
...
void BlurApp::UpdateMaterialCBs(const GameTimer& timer)
{
...
void BlurApp::UpdateMainPassCB(const GameTimer& timer)
{
...
void BlurApp::UpdateWaves(const GameTimer& timer)
{
...
void BlurApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<Renderable*>& renderables)
{
...
{
	const int textDescriptorCount = 3;
	const int blurDescriptorCount = 4;

	// Create the SRV heap.
	D3D12_DESCRIPTOR_HEAP_DESC srvHeapDesc = {};
	srvHeapDesc.NumDescriptors = textDescriptorCount + blurDescriptorCount;
	...
	m_BlurFilter->BuildDescriptors(
		CD3DX12_CPU_DESCRIPTOR_HANDLE(m_CbvSrvUavDescriptorHeap->GetCPUDescriptorHandleForHeapStart(), 3, m_CbvSrvUavDescriptorSize),
		CD3DX12_GPU_DESCRIPTOR_HANDLE(m_CbvSrvUavDescriptorHeap->GetGPUDescriptorHandleForHeapStart(), 3, m_CbvSrvUavDescriptorSize),
...
void BlurApp::BuildShadersAndInputLayout()                       
{
	...
	m_Shaders["HorzBlurCS"] = D3DUtil::CompileShader(L"Shaders/" + projectName + L"/Blur_CS.hlsl", nullptr, "HorzBlurCS", "cs_5_0");
	m_Shaders["VertBlurCS"] = D3DUtil::CompileShader(L"Shaders/" + projectName + L"/Blur_CS.hlsl", nullptr, "VertBlurCS", "cs_5_0");
